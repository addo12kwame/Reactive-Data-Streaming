...
def fetch_video_page(google_api_key,video_id,page_token=None):
    response = requests.get('https://www.googleapis.com/youtube/v3/videos', params={"key": google_api_key,
                                                                                       'id': video_id,
...
def main():
    ...
    kafka_config = config['kafka'] | {
        ...
        'value.serializer': AvroSerializer(schema_registry_client=schema_registry_client, schema_str=youtube_videos_value_schema.schema.schema_str)
    ...
    for video_item in fetch_playlist_items(google_api_key,youtube_playlist_id):
        ...
        for video in fetch_videos(google_api_key,video_id):
            logging.info('Got %s', summarize_video(video))
